...
import { getPublishedUserQuestions,  getUserSkills } from "../../api/api";
import { useSelector, useDispatch } from "react-redux";
import { getAllQuestions, getPagination, getUserQuestions, setPagination, setQuestions, setUserQuestions } from "../../redux/questionSlice";
import { ASK_NEW_QUESTION, DRAFTS, QUESTIONS_PAGE } from "../../util/Routes";
...
const UserQuestions = () => {
  ...
  const quizes = useSelector(getUserQuestions);
...
useEffect(()=>{
  const fetchQuestionByUserSkills = async () =>{
    try {
      ...
      const res = await getPublishedUserQuestions(profile.id,queryParams)
      dispatch(setUserQuestions(res.data.questions || []))
  ...
  return (
    <div className="w-[calc(100vw-336px)] bg-ibmlight p-6">
      ...
      <div className="flex items-center justify-between pb-4">
        <div><h1 className="text-xl font-bold text-ibmblue ">My Questions</h1></div>
        <div className="flex items-center gap-4">
         <h1 onClick={()=>navigate(QUESTIONS_PAGE)} className="flex gap-1 text-ibmblue font-bold cursor-pointer">All Questions <PiQuestionFill size={24}/></h1> 
