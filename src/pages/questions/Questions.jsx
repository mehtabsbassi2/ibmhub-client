...
const Questions = () => {
  ...
  const [pagination, setPaginationState] = useState(null);
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      setLoading(true);
      try {
        // 🔥 FIX: Build params object correctly
        const params = {
          params: {
            search,
            difficulty,
        ...
        // 🔥 FIX: Add userId to the params object, not append
        if (profile?.id) {
          params.params.userId = profile.id;
        }
    ...
    fetchQuestions();
  }, [search, difficulty, tag, status, sortBy, currentPage, profile?.id]);
  ...
  useEffect(() => {
    const fetchQuestionByUserSkills = async () => {
      try {
        ...
        console.error("Error fetching questions by skills:", error);
        setLoading(false);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="w-[calc(100vw-336px)] bg-ibmlight p-6">
      ...
      <div className="flex items-center justify-between pb-4">
        <div>
          <h1 className="text-xl font-bold text-ibmblue ">Browse Questions</h1>
        </div>
        <div className="flex items-center gap-4">
          <h1
            onClick={() => navigate(USER_QUESTIONS)}
            className="flex gap-1 text-ibmblue font-bold cursor-pointer"
          >
            My Questions <PiQuestionFill size={24} />
          </h1>
          <h1
            onClick={() => navigate(DRAFTS)}
            className="flex gap-1 text-ibmblue font-bold cursor-pointer"
          >
            Drafts <MdOutlineDrafts size={24} />
          </h1>
        </div>
      </div>
      ...
      <div className="flex items-center ">
        ...
        <div className="w-1/4 flex justify-end">
          <button
            onClick={handleAsk}
            className="btn btn-outline bg-ibmlight border-ibmblue hover:bg-ibmblue text-ibmblack hover:text-ibmlight transition-all ease-in-out duration-500"
          >
            Ask Question
          </button>
        </div>
